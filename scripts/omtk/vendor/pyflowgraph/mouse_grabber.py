...
from omtk.vendor.Qt import QtGui, QtCore
...
class MouseGrabber(PortCircle):
    ...
    def __init__(self, graph, pos, otherPortCircle, connectionPointType):
        super(MouseGrabber, self).__init__(
            None, graph, 0, otherPortCircle.getPort().getColor(), connectionPointType
        )
    ...
    def mouseMoveEvent(self, event):
        ...
        collidingPortItems = list(
            filter(lambda item: isinstance(item, (PortCircle, PortLabel)), collidingItems)
        )
