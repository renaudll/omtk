...
_g_preferences = preferences.preferences
...
class WidgetWelcome(QtWidgets.QWidget):
    ...
    def get_selected_rig_definition(self):
        row = next(
            iter(row.row() for row in self.rig_def_view.selectionModel().selectedRows()), None
        )
        if row:
    ...
    def set_selected_rig_definition(self, rig_def):
        ...
        except ValueError:  # ValueError: None is not in lis
            return
