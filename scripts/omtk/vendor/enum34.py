...
def _is_dunder(name):
    ...
    return (
        name[:2] == name[-2:] == "__" and name[2:3] != "_" and name[-3:-2] != "_" and len(name) > 4
    )
    ...
    def __new__(metacls, cls, bases, classdict):
        ...
        if _order_ is None:
            if pyver < 3.0:
                try:
                    _order_ = [
                        name for (name, value) in sorted(members.items(), key=lambda item: item[1])
                    ]
                except TypeError:
...
if pyver < 2.6:
    ...
    def __le__(self, other):
        raise TypeError(
            "unorderable types: %s() <= %s()" % (self.__class__.__name__, other.__class__.__name__)
        )
