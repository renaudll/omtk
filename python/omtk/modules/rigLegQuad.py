...
from omtk.modules import module_ik, rigLeg
from omtk.modules import rigLimb

log = logging.getLogger('omtk')


class CtrlIkQuadSwivel(module_ik.CtrlIkSwivel):
    """
...
class LegIkQuad(rigLeg.LegIk):
    """
    Quadruped libs system setup which inherit from the basic libs system
    """
    SHOW_IN_UI = False
    ...
    def create_ik_handle(self):
        """
        Override parent function to create a ikSpringSolver libs handle
    ...
    def create_ctrl_swivel(self, base_ctrl, ref, pos, ik_handle, constraint=True, mirror_setup=True,
        ...
        """
        Create the swivel ctrl for the libs system. Redefined to add the possibility to create a mirror swivel setup
        ...
        """
        # Do not contraint the libs handle now since we could maybe need the flipping setup
    ...
    def build(self, constraint=True, constraint_handle=True, setup_softik=True, **kwargs):
        ...
        :param constraint_handle: Bool to tell if we will contraint the handle on the libs ctrl
        :param setup_softik: Bool to tell if we setup the soft libs system
        ...
        # Hack: Re-parent ehe libs chain as a workaround for the spring libs solver bug.
        # Otherwise the current parent (which is constrained) will trigger and old sprint libs solver bug
        ...
        # Create a second libs chain for the quadruped setup
        ...
        for i, oIk in enumerate(self._chain_quad_ik):
            ...
            # Constraint the bones after the iCtrlIdx to the first libs chain to make the foot roll work correctly
        ...
        # We need a second libs solver for the quad chain
        ...
        # self.quad_swivel_distance = self.chain_length  # Used in libs/fk switch
