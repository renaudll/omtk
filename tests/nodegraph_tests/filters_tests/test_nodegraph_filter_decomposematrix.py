...
from omtk.vendor.maya_mock.decorators import mock_pymel
...
def add_intermediate_decomposeMatrix(session):
    ...
    with mock_pymel(session) as pymel:
        ...
        pymel.connectAttr(n1.matrix, n3.inputMatrix)
...
@pytest.mark.xfail
def test_unpredictable_decomposematrix(
    session, registry, model, cmds, add_intermediate_decomposeMatrix
):
    """
...
@pytest.mark.xfail
def test_unpredictable_composematrix(
    session, registry, model, cmds, add_intermediate_composeMatrix
):
    """
