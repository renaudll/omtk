...
class OmtkNodeGraphBasePortWidget(QtWidgets.QGraphicsWidget):
    ...
    def sceneEventFilter(self, watched, event):
        ...
        if event.type() == QtCore.QEvent.Type.GraphicsSceneMouseDoubleClick:
            ...
            #     self._ctrl.rename_port(self._session, new_name)
            #
    ...
    def _show_rename_delegate(self):
        # todo: share code with node.py
        from omtk.nodegraph.delegates.rename import NodeRenameDelegate
        ...
        def submit_callback(new_name):
            print(new_name)
            # self._ctrl.rename_node(self._session, new_name)
