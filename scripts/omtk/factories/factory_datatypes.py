...
def get_node_color_from_datatype(datatype):
    if datatype in (AttributeType.Component, AttributeType.Module):
        return QtGui.QColor(170, 128, 170, 255)
    if datatype in (AttributeType.Node,):
...
def get_attr_datatype(attr):
    ...
    if attr_type is None:
        raise Exception("Cannot resolve attribute type from attribute {0}.".format(attr))
