...
class NodeTitle(QtWidgets.QGraphicsWidget):
    ...
    def textSize(self):
        return QtCore.QSizeF(
            self.__textItem.textWidth(), self.__font.pointSizeF() + self.__labelBottomSpacing
        )
...
class Node(QtWidgets.QGraphicsWidget):
    ...
    def getGraphPos(self):
        ...
        return QtCore.QPointF(
            transform.dx() + (size.width() * 0.5), transform.dy() + (size.height() * 0.5)
        )
    ...
    def setGraphPos(self, graphPos):
        ...
        self.setTransform(
            QtGui.QTransform.fromTranslate(
                graphPos.x() - (size.width() * 0.5), graphPos.y() - (size.height() * 0.5)
            ),
            False,
        )
    ...
    def paint(self, painter, option, widget):
        ...
        painter.drawRoundedRect(
            0, 0, rect.width(), titleHeight, roundingX, roundingY, QtCore.Qt.AbsoluteSize
        )
