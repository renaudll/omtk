...
class Module2(Entity):
    ...
    def build(self, create_grp_anm=True, create_grp_rig=True, grp_anm_name=None, grp_rig_name=None,
        ...
        self.grp_root = pymel.createNode(
            'transform',
            name=nomenclature.resolve()
        )

        self.grp_inn = ModuleInputHub()
        self.grp_inn.build(
            name=nomenclature.resolve('inn'),
            parent=self.grp_root
        )

        self.grp_out = ModuleOutputHub()
        self.grp_out.build(
            name=nomenclature.resolve('out'),
            parent=self.grp_root
        )

        self.grp_anm = pymel.createNode(
            'transform',
            name=nomenclature.resolve('anm'),
            parent=self.grp_root
        )

        self.grp_rig = pymel.createNode(
            'transform',
            name=nomenclature.resolve('rig'),
            parent=self.grp_root
        )

        self.grp_hook = pymel.createNode(
            'transform',
            name=nomenclature.resolve('hook'),
            parent=self.grp_rig
        )
