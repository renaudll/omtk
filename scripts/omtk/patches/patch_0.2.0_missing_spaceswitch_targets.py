...
def patch_spaceswitch_data_into_network(sel=None):
    ...
    networks = libSerialization.get_connected_networks(
        sel, key=lambda net: libSerialization.is_network_from_class(net, "BaseCtrl")
    )
    ...
    for net in networks:
        ...
        if data:
            ...
            if not net.hasAttr("targets_indexes"):
                log.info(
                    "targets_indexes attribute is missing on network {0}. It will be created".format(
                        net
                    )
                )
            ...
            if not net.hasAttr("targets"):
                log.info(
                    "targets attribute is missing on network {0}. It will be created".format(
                        net
                    )
                )
            ...
            for index, (name, obj) in data.items():
                ...
                if obj not in target_data:
                    if index not in indexes_data:
                        if obj is not None:
                            if isinstance(obj, pymel.nt.Joint):
                                log.info(
                                    "Patching network {0} space switch data named {1} - Index {2} on object {3}".format(
                                        net.name(), name, index, obj
                                    )
                                )
                                pymel.connectAttr(
                                ...
                                if ctrl_net > 0:
                                    log.info(
                                        ...
                                        "Index {2} on object {3}".format(
                                            net.name(), name, index, ctrl_net[0]
                                        )
                                    )
                                    pymel.connectAttr(
                                        ctrl_net[0].message,
                            ...
                            log.info(
                                "Patching network {0} space switch data named {1} - Index {2} as local index".format(
                                    net.name(), name, index
                                )
                            )
                    ...
                    log.info(
                        "Space Switch object {0} is already included in the targets attributes".format(
                            obj
                        )
                    )
...
def patch_spaceswitch_dpspine_objects():
    ...
    to_patch = [
        module
        for module in rig_instance.modules
        ...
        if isinstance(module_to_patch, rigDpSpine.DpSpine):
            ...
            for connection in connected_to_fk_dwn:
                ...
                if attr_name.find("targets") >= 0:
                    log.info(
                        "Reconnecting {0} from {1} to space switch node {2}".format(
                            connection,
                            module_to_patch.ctrl_ik_dwn,
