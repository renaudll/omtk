...
class NodeGraphView(PyFlowgraphView):
    ...
    def dropEvent(self, event):
        from omtk.vendor.omtk_compound import CompoundDefinition
        ...
        def _handle_component_definition(component_def):
            # type: (CompoundDefinition) -> Component
            return component_def.instanciate()

        drop_data = [
            _handle_component_definition(d) if isinstance(d, CompoundDefinition) else d
            for d in drop_data
        ]
