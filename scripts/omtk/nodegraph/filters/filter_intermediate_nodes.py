...
_g_composematrix_predictable_outputs_inv = {
    v: k for k, v in _g_composematrix_predictable_outputs.iteritems()
}
    ...
    def intercept_connection(self, connection):
        ...
        def _intercept_decomposematrix_connection():
            ...
            if port_dst_name == "inputMatrix" and not is_dst_node_visible:
                for sub_connection in self._get_decomposematrix_inputmatrix_output_connections(
                    registry, port_dst
                ):
        ...
        def _intercept_composematrix_connection():
            if port_dst_name in _g_composematrix_predictable_outputs and not is_dst_node_visible:
                ...
                for sub_connection in model.get_port_output_connections(redirection_port):
                    new_connection = registry.get_connection(
                        port_src, sub_connection.get_destination()
                    )
                    yield new_connection
                return
        ...
        if self.hide_predictable_decomposematrix_node:
            ...
            if (
                node_src_type == "composeMatrix"
                and not model.is_node_visible(node_src)
                and is_composematrix_connection_predictable(connection)
            ):
            ...
            if (
                node_dst_type == "composeMatrix"
                and not model.is_node_visible(node_dst)
                and is_composematrix_connection_predictable(connection)
            ):
                for yielded in _intercept_composematrix_connection():
                    yield yielded
        ...
        for attr_dst in node_model.get_connected_output_ports():
            ...
            for connection in model.get_port_output_connections(attr_dst):
                ...
                if is_decomposematrix_connection_predictable(connection):
                    ...
                    log.warning(
                        "Will no ignore {0} because of an unpredictable connection {1}.".format(
                            node_model, connection
                        )
                    )
                    yield port
                    return
