...
def get_port_from_value(registry, val):
    ...
    if isinstance(val, entity_attribute.EntityPymelPort):
        ...
        inst = omtk.nodegraph.models._deprecated.port_entityattribute.NodeGraphEntityAttributePortModel(
            registry, node_model, val
        )
        return inst
    ...
    if isinstance(val, pymel.Attribute):
        node_value = val.node()
        node_model = registry.get_node(node_value)
        inst = omtk.nodegraph.models._deprecated.port_pymel.NodeGraphPymelPortModel(
            registry, node_model, val
        )
        return inst
    ...
    if datatype == factory_datatypes.AttributeType.Node:
        ...
        inst = omtk.nodegraph.models._deprecated.port_pymel.NodeGraphPymelPortModel(
            registry, node_model, val.message
        )
        return inst
