...
def _get_mfndependencynode_from_dagpath(dagpath):
...
def _get_network_class(mfn):
    # type: (om2.MFnDependencyNode) -> str
    """
    Resolve the original class for a serialialized network.
    :param mfn: A MFnDependencyNode for a serialized network.
    :return: The fully qualified class name if applicable. None otherwise.
    """
    if mfn.hasAttribute('_class'):
