...
from omtk.vendor.omtk_compound import CompoundDefinition, Compound
...
def get_component_dir():
    ...
    return os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "compounds"))
...
def walk_available_component_definitions(search_scripted=True):
    ...
    assert os.path.exists(path_component_dir)
    ...
    for dirname in paths:
        ...
        if os.path.exists(dirname):
            for filename in os.listdir(dirname):
                ...
                component_def = CompoundDefinition.from_file(path)
...
def get_component_metanetwork_from_hub_network(network, strict=True):
    ...
    result = next(
        iter(libSerialization.get_connected_networks(network, key=_filter, recursive=False)), None
    )
...
def create_component_from_bounds(objs):
    ...
    inst = Compound.from_attributes(input_attrs, output_attrs)
    ...
    if map_inn:
        for attr_name, attr_val in map_inn.iteritems():
            if not inst.has_input_attr(attr_name):
                raise Exception(
                    "Component {0} don't have an {1} input attribute.".format(inst, attr_name)
                )
    ...
    if map_out:
        for attr_name, attr_val in map_out.iteritems():
            if not inst.has_output_attr(attr_name):
                raise Exception(
                    "Component {0} don't have an {1} output attribute.".format(inst, attr_name)
                )
