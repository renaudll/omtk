...
from omtk.vendor.omtk_compound import Compound
...
class NodeGraphComponentPortModel(
    ...
    def __init__(self, registry, node, attr_def):
        ...
        self._inn_adaptor = (
            entity.PymelAttributeNodeGraphPortImpl(grp_inn_attr) if grp_inn_attr else None
        )
        self._out_adaptor = (
            entity.PymelAttributeNodeGraphPortImpl(grp_out_attr) if grp_out_attr else None
        )
...
class NodeGraphComponentModel(node_entity.NodeGraphEntityModel):
    ...
    def __init__(self, registry, entity):
        assert isinstance(entity, Compound)
    ...
    def get_children(self):
        return [self._registry.get_node(pynode) for pynode in self._entity.get_children()]
