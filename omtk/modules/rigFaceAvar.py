...
from omtk.models.modelNonInteractiveCtrl import ModelNonInteractiveCtrl
...
EnumAvarCtrlTypes = Enum('Avar Ctrl Types', ['Linear', 'Surface'])
EnumModelCtrlTypes = Enum('Model Ctrl Types', ['Linear', 'Interactive'])
...
class AvarRigConnectionModelLinear(BaseAvarRigConnectionModel):
    """
    ...
    def __init__(self, *args, **kwargs):
        super(AvarRigConnectionModelLinear, self).__init__(*args, **kwargs)
    ...
    def build(self, constraint=True, ctrl_tm=None, jnt_tm=None, obj_mesh=None, follow_mesh=True,
        ...
        super(AvarRigConnectionModelLinear, self).build(parent=False)
...
class AvarRigConnectionModelSurface(AvarRigConnectionModelLinear):
    """
    ...
    def __init__(self, *args, **kwargs):
        super(AvarRigConnectionModelSurface, self).__init__(*args, **kwargs)
        self.surface = None
    ...
    def unbuild(self):
        ...
        super(AvarRigConnectionModelSurface, self).unbuild()
...
class AvarSimple(AbstractAvar):
    ...
    _CLS_MODEL_CTRL = ModelInteractiveCtrl  # todo: remove, we give the responsability to the rigger now
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.model_avar_type = EnumAvarCtrlTypes.Linear
        self.model_ctrl_type = EnumModelCtrlTypes.Linear
    ...
    def get_model_ctrl_class(self):
        if self.model_ctrl_type == EnumModelCtrlTypes.Linear:
            return ModelNonInteractiveCtrl
        if self.model_ctrl_type == EnumModelCtrlTypes.Interactive:
            return ModelInteractiveCtrl
        raise Exception("Unsupported CtrlModel type {0}".format(self.model_avar_type))
    ...
    def get_model_avar_class(self):
        if self.model_avar_type == EnumAvarCtrlTypes.Linear:
            return AvarRigConnectionModelLinear
        if self.model_avar_type == EnumAvarCtrlTypes.Surface:
            return AvarRigConnectionModelSurface
        raise Exception("Unsupported AvarModel type {0}".format(self.model_avar_type))
