...
@pytest.fixture
def port_src(cmds, node):
    cmds.addAttr(node, longName='testPort')
    return '%s.testPort' % node


@pytest.fixture
def port_dst(cmds, node):
    cmds.addAttr(node, longName='testPortDst')
    return '%s.testPortDst' % node


@pytest.fixture
def connection(cmds, node, port_src, port_dst):
    cmds.addAttr(node, longName='testPortDst')
    return cmds.connectAttr('a.testPort', 'a.testPortDst')
...
@pytest.fixture
def port_src_model(port_src, registry):
    return registry.get_port(port_src)


@pytest.fixture
def port_dst_model(port_dst, registry):
    return registry.get_port(port_dst)
...
def connection_model(connection, registry):
    ...
    return registry.get_connection('a.testPort', 'a.testPortDst')
...
def test_add_port(model, port_src_model):
    """Validate that we can add a port to the graph."""
    model.add_port(port_src_model)
    assert model.get_ports() == {port_src_model}


def test_add_port_automatically_add_node(model, node_model, port_src_model):
    """Validate that when we add a port to the graph it's node is also added."""
    model.add_port(port_src_model)
    assert model.get_nodes() == {node_model}


def test_remove_port(model, port_src_model):
    """Validate that we can remove a port from the graph."""
    model.add_port(port_src_model)
    model.remove_port(port_src_model)
    assert model.get_ports() == set()


def test_get_node_ports(model, node_model, port_src_model):
    """Validate we can access the ports associated with a node."""
    model.add_port(port_src_model)
    assert model.get_node_ports(node_model) == {port_src_model}


def test_is_port_visible(model, port_src_model):
    """Validate a port we add is visible by default."""
    model.add_port(port_src_model)
    assert model.is_port_visible(port_src_model)
...
def test_add_connection_automatically_add_port(model, connection_model, port_src_model, port_dst_model):
    """Validate when we add a connection to the graph the port is added."""
    model.add_connection(connection_model)
    assert model.get_ports() == {port_src_model, port_dst_model}
...
def test_on_node_unexpectedly_deleted(cmds, model, node, node_model):
...
def test_on_attribute_unexpectedly_removed(cmds, model, node, port_src, port_src_model):
    """Validate that when a port is removed from the session it is removed from the graph."""
    model.add_port(port_src_model)
    cmds.deleteAttr(node, attribute='testPort')
    assert model.get_ports() == set()


def test_on_connection_unexpectedly_added(model, port_src_model):
    """Validate that when a port is added to the session nothing happen."""
    assert model.get_ports() == set()


def test_on_connection_unexpectedly_removed(cmds, model, connection, connection_model):
    """Validate that when a connection is removed from the session it is removed from the graph."""
    model.add_connection(connection_model)
    cmds.disconnectAttr('a.testPort', 'a.testPortDst')
    assert model.get_connections() == set()


def test_expand_node_ports(model, node_model, port_src_model):
...
def test_expand_node_connections(model, node_model, port_src_model, connection_model):
...
def test_add_all(model, node_model, port_src_model, port_dst_model, connection_model):
    """Ensure that we can add all registered node, port and connection."""
    model.add_all()
    assert model.get_nodes() == {node_model}
    assert model.get_ports() == {port_dst_model, port_src_model}
    assert model.get_connections() == {connection_model}
