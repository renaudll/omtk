...
import omtk
from omtk.vendor.maya_mock import MockedCmdsSession, MockedPymelSession, MockedSessionSchema
from omtk.nodegraph import GraphModel, GraphFilterProxyModel, NodeGraphRegistry
...
# For our mocked maya, we will use a pre-generated schema.
path_omtk = os.path.dirname(omtk.__file__)
path_schema = os.path.abspath(os.path.join(path_omtk, '..', '..', 'tests', 'resources', 'schema_2017.json'))
PRECONFIGURED_SCHEMA = MockedSessionSchema.from_json_file(path_schema)
...
class NodeGraphBaseTestCase(OmtkTestCase):
    ...
    def _create_session(self):
        return self._cls_session() if self._cls_session is not None else None

    def _create_registry(self):
        return self._cls_registry()
    ...
    def setUp(self):
        ...
        self.model = self._cls_proxy_model(registry=self.registry,
                                           model=self.source_model) if self._cls_proxy_model else self.source_model
...
class NodeGraphMockedMayaTestCase(NodeGraphBaseTestCase):  # TODO: Deprecate in favor of pytest fixtures
    _cls_session = MockedSession

    _cls_session_preset = PRECONFIGURED_SCHEMA  # REGISTRY_DEFAULT  # This one is an object instead of a class???
    ...
    def _create_registry(self):
        return self._cls_registry(self.session)
