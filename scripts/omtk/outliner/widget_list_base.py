...
from omtk.core import constants_ui
...
class OmtkBaseListWidget(QtWidgets.QWidget):
    ...
    def get_selected_modules(self):
        ...
        return [
            item._meta_data
            for item in self.iter_selected_items()
            if item._meta_type == constants_ui.MimeTypes.Module
        ]
    ...
    def get_selected_rigs(self):
        ...
        return [
            item._meta_data
            for item in self.iter_selected_items()
            if item._meta_type == constants_ui.MimeTypes.Rig
        ]
    ...
    def get_selected_components(self):
        ...
        return [
            item._meta_data
            for item in self.iter_selected_items()
            if isinstance(item._meta_data, Entity)
        ]
        ...
        for qt_item in libQt.get_all_QTreeWidgetItem(self.ui.treeWidget):
            if hasattr(qt_item, "rig"):
                qt_item.setCheckState(
                    0, QtCore.Qt.Checked if qt_item._meta_data.is_built else QtCore.Qt.Unchecked
                )
    ...
    def on_context_menu_request(self, pos):
        ...
        selected_items = self.ui.treeWidget.selectedItems()
        selected_components = [
            item._meta_data for item in selected_items if isinstance(item._meta_data, Entity)
        ]
