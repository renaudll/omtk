...
def arrange_upstream(node, padding_x=32, padding_y=32):
    ...
    for (child,) in zip(children):
        ...
        child.setGraphPos(
            QtCore.QPointF(pos_x - ((child.size().width() - total_width) / 2.0), pos_y,)
        )
...
def arrange_downstream(node, padding_x=32, padding_y=10):
    ...
    for i, child, in enumerate(children):
        ...
        log.debug("Repositionning {} ({}) to {}, {}".format(child, child.getName(), pos_x, pos_y))
        child.setGraphPos(
            QtCore.QPointF(pos_x + ((child.size().width() - total_width) / 2.0), pos_y)
        )
...
def spring_layout(nodes):
    ...
    # Adjust scale
    new_positions = [
        QtCore.QPointF(x, y) for i, (x, y) in zip(range(num_vertices), layout.itervalues())
    ]
