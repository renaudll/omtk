...
class NodeGraphController(object):  # QtCore.QObject is necessary for signal handling
    ...
    def __init__(self, registry, model=None, view=None):
        ...
        self._buffer_old_nodes = (
            set()
        )  # nodes that the graph will try to display when the model is reset
    ...
    def get_port_widget(self, port):
        ...
        port_widget = port.get_widget(
            self, self._view, node_widget, is_input=is_input, is_output=is_output
        )
    ...
    def get_connection_widget(self, connection_model):
        ...
        if not widget_dst_node_in_circle:
            raise Exception(
                "Expected an inCircle widget for destination when connecting {0}.{1} to {2}.{3}".format(
                    widget_src_node.getName(),
                    widget_src_port.getName(),
                    widget_dst_node.getName(),
                    widget_dst_port.getName(),
                )
            )
        ...
        try:
            ...
            log.warning(
                "Error connecting {0} to {1}, {2}".format(
                    port_src_model.get_name(), port_dst_model.get_name(), e
                )
            )
    ...
    def on_right_click(self, menu):
        ...
        values = [
            v for v in values if isinstance(v, entity.Entity)
        ]  # limit ourself to _known_definitions
    ...
    def _add_actions_for_component(self, menu, inst):
        ...
        """
        from omtk.vendor.omtk_compound import manager
            ...
            if len(component_versions) > 1:
                ...
                for component_version in component_versions:
                    ...
                    menu_action.triggered.connect(
                        functools.partial(self.update_selected_nodes_to, component_version)
                    )
        ...
        for node in nodes:
            ...
            for connection in model.get_port_input_connections(node):
                ...
                if not isinstance(metadata, pymel.Attribute):
                    log.warning(
                        "Ignoring connection %s, invalid destination metadata type. "
            ...
            for connection in model.get_port_output_connections(node):
                ...
                if not isinstance(metadata, pymel.Attribute):
                    log.warning(
                        "Ignoring port %s, invalid source metadata type. "
                        "Expected pymel.Attribute, got %s." % (connection, metadata)
                    )
                    continue
    ...
    def _update_node_to_latest_version(self, node):
        ...
        if not latest_def:
            log.warning(
                "Found no version available for {0} ({1})".format(cmpnt_def.name, cmpnt_def.uid)
            )
            return
    ...
    def group_nodes(self, nodes):
        ...
        """
        from omtk.vendor.omtk_compound.core import _factory
        registry = self.get_registry()
        ...
        inst = _factory.from_attributes(map_inn, map_out, dagnodes=dgnodes)
    ...
    def on_show_properties_panel(self):
        ...
        inst = next(
            (val for val in self.get_selected_values() if isinstance(val, component.Component)),
            None,
        )
