...
class ComponentGradientFloat(ComponentScripted):
    ...
    def build_interface(self):
        ...
        self.attr_inn_value_s = self.add_input_attr(
            self.ATTR_NAME_INN_VALUE_S, defaultValue=0.0, at="float"
        )
        self.attr_inn_value_e = self.add_input_attr(
            self.ATTR_NAME_INN_VALUE_E, defaultValue=1.0, at="float"
        )
        self.attr_num_values = self.add_input_attr(
            self.ATTR_NAME_INN_NUM_VALUES, at="byte", defaultValue=3
        )
        self.attr_out_values = self.add_output_attr(
            self.ATTR_NAME_OUT_VALUES, multi=True, at="float"
        )
    ...
    def build_content(self):
        ...
        for i in xrange(num_steps):
            ...
            u = libRigging.create_utility_node(
                "blendTwoAttr",
                input=(self.attr_inn_value_s, self.attr_inn_value_e,),
                attributesBlender=ratio,
            )
            pymel.connectAttr(u.output, self.attr_out_values[i])
