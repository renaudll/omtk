...
class SubgraphProxyModel(graph_proxy_model.NodeGraphGraphProxyModel):
    ...
    def set_level(self, level):
        ...
        node_inn_bound = NodeGraphComponentInnBoundModel(
            registry, node_inn_model._impl
        )
        ...
        node_out_bound = NodeGraphComponentOutBoundModel(
            registry, node_out_model._impl
        )
    ...
    def get_connection_parent(self, connection):
        ...
        class ConnectionKind(Enum):
            ...
            compound_inn_to_compound_out = (
                7  # any (source and destination are inside the same compound)
            )
            compound_out_to_compound_inn = (
                8  # any (source and destination are inside the same compound)
            )
