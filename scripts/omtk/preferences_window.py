...
class PreferencesWindow(QtWidgets.QDialog):
    ...
    def __init__(self, parent=None):
        ...
        # Fill the QComboBox
        self.rig_plugins = sorted(
            plugin_manager.plugin_manager.get_loaded_plugins_by_type("rigs")
        )
        rig_plugins_names = [
            plugin.cls.__name__ for plugin in self.rig_plugins if plugin
        ]
        ...
        if default_rig_type_name in rig_plugins_names:
            self.ui.comboBox.setCurrentIndex(
                rig_plugins_names.index(default_rig_type_name) + 1
            )
    ...
    def on_default_rig_changed(self, index):
        if index == 0:
            ...
            preferences.preferences.default_rig = self.rig_plugins[
                index - 1
            ].cls.__name__
