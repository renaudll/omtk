...
class ComponentCache(object):
    ...
    def add_component_to_cache(self, component):
        ...
        for metadata in pymel.PyNode(component.input).message.outputs():
            self._component_by_metadata[metadata] = component
            self._cache_parent_by_node[metadata] = component

        self._component_network_by_hub_inn[component.input] = component
        self._component_network_by_hub_out[component.output] = component
...
class AutoRigManager(QtCore.QObject):
    ...
    def import_components(self):
        ...
        results = [
            libSerialization.import_network(network, module="omtk", cache=self._cache_serialization)
