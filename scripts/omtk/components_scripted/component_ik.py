...
def _get_chain_length_from_local_tms(tms):
    ...
    distances = [
        libRigging.create_utility_node(
            "distanceBetween", inMatrix1=attr_parent_tm, inMatrix2=attr_child_tm
        ).distance
        for attr_parent_tm, attr_child_tm in libPython.pairwise(tms)
    ]

    return libRigging.create_utility_node("plusMinusAverage", input1D=distances).output1D
...
def _create_joints_from_local_tms(tms):
    def _fn(tms):
        ...
        for tm in tms:
            ...
            u = libRigging.create_utility_node("decomposeMatrix", inputMatrix=tm)
...
class ComponentSoftIk(ComponentScripted):
    ...
    def build_content(self):
        ...
        formula.distanceSoftClamped = libRigging.create_utility_node(
            "clamp", inputR=formula.distanceSoft, minR=0.0001, maxR=999
        ).outputR
        ...
        formula.outDistance = libRigging.create_utility_node(
            ...
            attributesBlender=formula.inStretch,
        ).output
        ...
        formula.outStretch = libRigging.create_utility_node(
            "blendTwoAttr", input=[1.0, formula.outStretch], attributesBlender=formula.inStretch
        ).output
...
class ComponentIkBuilder(DagBuilder):
    ...
    def create_ik_handle(self, obj_s, obj_e, solver_type="ikRPsolver"):
        ...
        ik_handle, ik_effector = pymel.ikHandle(
            startJoint=obj_s, endEffector=obj_e, solver=solver_type
        )
    ...
    def setup_softik(self):
        ...
        attr_current_s_tm = self.create_utility_node(
            "multMatrix", matrixIn=(self.attr_inn_chain_tms[0], self.attr_inn_hook)
        ).matrixSum
        util_decompose_current_s = self.create_utility_node(
            "decomposeMatrix", inputMatrix=attr_current_s_tm
        )
        attr_current_s_pos = util_decompose_current_s.outputTranslate

        # todo: use decorator?
        attr_current_e_pos = self.create_utility_node(
            "decomposeMatrix", inputMatrix=self.attr_inn_ctrl_tm
        ).outputTranslate
...
class ComponentIk(ComponentScripted):
    ...
    def build_interface(self):
        ...
        self._attr_ctrl_swivel_pos = self.add_input_attr(
            "swivelPos", dt="double3"
        )  # translate datatype
