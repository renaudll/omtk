...
class ComponentFkBuilder(DagBuilder):
    ...
    def build(self):
        ...
        for attr_ctrl_bind, attr_inf_bind in zip(self._attr_anm_bind, self._attr_inf_bind):
            offset_tms.append(self.get_local_tm(attr_inf_bind, attr_ctrl_bind))
        ...
        for offset_tm, ctrl_tm in zip(offset_tms, self._attr_anm_ctrl):
            out_tm = self.create_utility_node(
                "multMatrix", matrixIn=(offset_tm, ctrl_tm, self._attr_hook)
            ).matrixSum
...
class ComponentFk(ComponentScripted):
    ...
    def build_interface(self):
        ...
        self._attr_inn_chain = self.add_input_attr(
            self.ATTR_NAME_INN_INF_BIND, dt="matrix", multi=True
        )
        self._attr_ctrls_binPose = self.add_input_attr(
            self.ATTR_NAME_INN_ANM_BIND, dt="matrix", multi=True
        )
        self._attr_ctrl_tms = self.add_input_attr(
            self.ATTR_NAME_INN_ANM_TMS, at="matrix", multi=True
        )
        self._attr_inn_hook_tm = self.add_input_attr(self.ATTR_NAME_INN_PARENT_TM, at="matrix")
        self._attr_out_matrices = self.add_output_attr(
            self.ATTR_NAME_OUT_INF_TMS, at="matrix", multi=True
        )
