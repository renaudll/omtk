...
import omtk.core
import omtk.core.constants
from omtk.core import constants, preferences, manager
...
class OmtkUI(main_window_extended.MainWindowExtended):
    ...
    def __init__(self, parent=None):
        ...
        version = omtk.core.constants.get_version()
    ...
    def set_current_widget(self, index):
        ...
        old_positions = [
            QtCore.QRect(width * (i - old_index), 0, width, height) for i in range(num_widgets)
        ]
        new_positions = [
            QtCore.QRect(width * (i - index), 0, width, height) for i in range(num_widgets)
        ]

        self._qt_animation = (
            QtCore.QParallelAnimationGroup()
        )  # member variable necessary to bypass garbage collection
