...
class CompoundModule(Module):
    ...
    def create_compound_ctrl(
        self, cls, inst, suffix, attr_inn_name, attr_out_name, **kwargs
    ):
        """
    ...
    def create_compound_influence(self, suffix, attr_name, target=None, **kwargs):
        ...
        """
        inst = pymel.createNode(
            "joint", name=self.get_nomenclature_jnt().resolve(suffix)
        )
    ...
    def build(self, create_grp_inf=True, **kwargs):
        ...
        if create_grp_inf:
            self.grp_influences = pymel.createNode(
                "transform", name=nomenclature_rig.resolve("influences")
            )
